...
- [x] 7.6 Keys de usuario cargadas y validadas (fila 98 completa): VirusTotal nueva key OK (credencial recreada `znTR0Vhc1NPWwvmQ`, probe 200). WhoisFreaks: la key era válida — el servicio MIGRÓ a `/v2.0/whois/live?apiKey&whois=live&domainName` (v1.0 muerto con 404 ISE incluso en root); credencial recreada `1z0sMIFlttji5qxS`. Fixes extra: `String.match()` global contaminaba dominios con esquema (`http://github.com` → matchAll con grupo de captura); WHOIS recibía como input la RESPUESTA de VirusTotal (sin `extracted_domains`) → domainName siempre vacío → fix con `$('Dividir Dominios').all()[$itemIndex]`; scoring lee `create_date` (v2) con fallback. Pendiente operativo: rotar las 3 keys expuestas en chat (AbuseIPDB, VirusTotal, WhoisFreaks) tras esta verificación.
